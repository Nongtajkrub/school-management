...
static bool send_none_block(i32 sockfd, const void* buf, usize size) {
	...
	while (total_sent != size) {
		ssize sent = send(
			sockfd,
		   	(const char*)buf + total_sent, size - total_sent, MSG_DONTWAIT);
...
bool netio_send(i32 sockfd, const void* buf, usize size, bool block) {
	return (block) ? 
		send_block(sockfd, buf, size) : send_none_block(sockfd, buf, size);
}
...
bool netio_recv(i32 sockfd, void *buf, usize size, bool block) {
	return (block) ? 
		recv_block(sockfd, buf, size) : recv_none_block(sockfd, buf, size);
}
